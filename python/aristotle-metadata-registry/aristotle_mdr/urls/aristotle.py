...
urlpatterns=[
    ...
    url(r'^item/(?P<iid>\d+)(?:\/(?P<model_slug>\w+)\/(?P<name_slug>.+))?/?$', views.Concept.as_view(), name='item'),
    url(r'^item/(?P<iid>\d+)(?:\/.*)?$', views.Concept.as_view(), name='item'),  # Catch every other 'item' URL and throw it for a redirect
